...
layout = html.Div([
    ...
    html.Div([
        dcc.Markdown("""
        [GitHub Project](https://github.com/MaxTechniche/BuildWeek2)
        ...
        I chose a ridge model to test next. That had pretty much the same accuracy \(that's the model doing the predicting btw\), so my theory is that I don't have good/enough features to accurately predict a movies runtime. I personally think that there are a lot of features that don't really help in predicting the runtime of movies. There are also a few features that are skewed.

        I could look for more features/info about movies in order to predict runtime, which could theoretically help in determining how long a movie should be to more likely get the desired feature values. But until then, here are the feature coefficients of the current model.
        """),
